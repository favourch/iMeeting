...
function addCalendar($st, $et, $sub, $ade){
  if(Yii::app()->user->id){	
  	...
  	Yii:redirect(array('wdcalendar'));
  }
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `jqcalendar` (`user_id`, `subject`, `starttime`, `endtime`, `isalldayevent`) values ("
     	.$user_id.", '"
...
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $color, $tz){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `jqcalendar` (`user_id`, `subject`, `starttime`, `endtime`, `isalldayevent`, `description`, `location`, `color`) values ("
      .$user_id.", '"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
