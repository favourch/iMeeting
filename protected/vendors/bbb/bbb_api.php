...
class BigBlueButton {
	...
	var $salt = "d6cc20d3f12db6ea98df7b4f8da16ccd";

	// This is the URL for the BigBlueButton server 
	//Make sure the url ends with /bigbluebutton/
	var $url = "http://hcm.imeeting.vn/bigbluebutton/";
	...
	public function getChecksumFromUrl($url){
	...
	*/
	public function createMeetingArray( $username, $meetingID, $welcomeString, $mPW, $aPW, $SALT, $URL, $logoutURL ) {
		...
		$url_created = BigBlueButton::createMeetingURL($username, $meetingID, $aPW, $mPW, $welcomeString, $logoutURL, $SALT, $URL );
		
		$xml = bbb_wrap_simplexml_load_file($url_created );
...
//-------------------------------------------getMeetingInfo---------------------------------------------------
	...
	*	If success it returnsan xml packet containing a returncode, 
	*/
	public function getMeetingInfoIntoObject( $meetingID, $modPW, $URL, $SALT ) {
		$xml = bbb_wrap_simplexml_load_file( BigBlueButton::getMeetingInfoURL( $meetingID, $modPW, $URL, $SALT ) );
		if($xml){
			return $xml;
		}
	...
	*/
	 ...
	 * 
	 */
	public function countUsers( $meetingID, $modPW, $URL, $SALT, $UNAME = false ) {
		$xml = bbb_wrap_simplexml_load_file( BigBlueButton::getMeetingInfoURL( $meetingID, $modPW, $URL, $SALT ) );
		if( $xml && $xml->returncode == 'SUCCESS' ) {
			
			if( count( $xml->attendees ) && count( $xml->attendees->attendee ) ) {
				
				return count( $xml->attendees->attendee ) ;
			}
