...
class Videos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, type, content, image', 'required'),
			array('type', 'numerical', 'integerOnly'=>true),
			array('name, image', 'length', 'max'=>500),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, type, content, image', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getTop($number){
		$videos = Yii::app()->db->createCommand()
  			  ->select('id, name, image, content, type')
    			->from('tbl_videos v')
    			->order('rand()')
				->limit($number)
    			->queryAll();
	...
	public function getTopRand($number){
		$videos = Yii::app()->db->createCommand()
  			  ->select('id, name, image, content, type')
    			->from('tbl_videos v')
    			->order('id desc')
